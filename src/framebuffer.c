    ...
    const VkBindDescriptorSetsInfoKHR *pBindDescriptorSetsInfo)
{
    ...
    {
        STEREO_LOG(
            "RT_DESC_BIND2KHR cb=%p firstSet=%u count=%u stageFlags=0x%08X",
            (void*)commandBuffer,
            pBindDescriptorSetsInfo->firstSet,
            pBindDescriptorSetsInfo->descriptorSetCount,
            pBindDescriptorSetsInfo->stageFlags);
        if (pBindDescriptorSetsInfo->pDescriptorSets &&
            pBindDescriptorSetsInfo->firstSet == 0 &&
            pBindDescriptorSetsInfo->descriptorSetCount > 0 &&
            sd)
        {
            ...
            if (slot != UINT32_MAX)
            {
                sd->rt_desc_sets[slot] = pBindDescriptorSetsInfo->pDescriptorSets[0];
                sd->rt_desc_cmd_first_set[slot] = 0;
                STEREO_LOG(
                    "RT_DESC_SET0_TRACK2KHR cb=%p set=%p slot=%u",
                    (void*)commandBuffer,
                    (void*)(uintptr_t)pBindDescriptorSetsInfo->pDescriptorSets[0],
                    slot);
            }
        }
    }
    if (!sd || !sd->real.CmdBindDescriptorSets2KHR)
        return;
    sd->real.CmdBindDescriptorSets2KHR(
        commandBuffer,
    ...
    for (uint32_t ti = 0; ti < sd->rt_desc_template_count; ti++)
    {
        ...
        for (uint32_t ei = 0; ei < t->entry_count; ei++)
        {
            ...
            for (uint32_t di = 0; di < e->descriptorCount && di < 4; di++)
            {
                ...
                base + e->offset + (size_t)e->stride * di;
                    ...
                    e->descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
                {
                    ...
                    (const VkDescriptorImageInfo *)entry_data;
                    ...
                    e->descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC)
                {
                    ...
                    STEREO_LOG(
                        ...
                        e->dstBinding,
                        di,
                        (void*)(uintptr_t)bi->buffer,
                        (unsigned long long)bi->offset,
                        (unsigned long long)bi->range);
                }
                else if (e->descriptorType == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR)
                {
                    const VkAccelerationStructureKHR *as =
                    (const VkAccelerationStructureKHR *)entry_data;
                    STEREO_LOG(
                        "RT_TEMPLATE_VALUE set=%p entry=%u binding=%u index=%u acceleration_structure=%p",
                        (void*)(uintptr_t)descriptorSet,
                        ei,
                        e->dstBinding,
                        di,
