...
bool build_passthrough_gs_spv(const uint32_t *vs_spv, size_t vs_wc,
                              uint32_t **out, size_t *out_c)
{
    /* Simple pass-through GS for VS-only TRIANGLE_LIST pipelines.
     *
     * NVIDIA 426.06: gl_ViewIndex in TES is only populated when a GS follows.
     * This GS forces that condition.  It does NOT write gl_Layer — the driver
     * handles multiview layer routing automatically when gl_Layer is unset,
     * exactly as it does for the app's own geometry-shader pipelines.
     *
     * Writing gl_Layer = gl_ViewIndex from a synthesized GS does NOT work
     * because gl_ViewIndex inside our synthesized GS is always 0 on 426.06,
     * causing both views to write to layer 0 and overlap on the left eye. */
    ...
    /* gl_PerVertex in/out (Position only — no gl_Layer, no gl_ViewIndex) */
    uint32_t id_pv_in=nid++, id_pv_out=nid++;
    uint32_t id_c3=nid++;
    uint32_t id_arr_in=nid++;  /* array(pv_in, 3) for gl_in triangles */

    /* Pointer types */
    uint32_t id_pIarr=nid++;   /* ptr Input  array(pv_in,3) */
    uint32_t id_pOPV =nid++;   /* ptr Output pv_out         */
    uint32_t id_pIv4 =nid++;   /* ptr Input  vec4           */
    ...
    /* User varying vector types (new: vec2, vec3; vec4/float reused) */
    uint32_t id_uvec[5]={0};
    id_uvec[1]=id_float; id_uvec[4]=id_v4;
    if(need_vs[2]) id_uvec[2]=nid++;
    if(need_vs[3]) id_uvec[3]=nid++;

    /* User varying array input types: array(T, 3) */
    uint32_t id_uarr3[5]={0};
    for(int vs=1;vs<=4;vs++) if(need_vs[vs]) id_uarr3[vs]=nid++;

    /* User varying ptr types */
    uint32_t id_pI_uvec[5]={0}, id_pO_uvec[5]={0}, id_pI_uarr3[5]={0};
    id_pI_uvec[4]=id_pIv4; id_pO_uvec[4]=id_pOv4;
    for(int vs=1;vs<=4;vs++){
        if(!need_vs[vs]) continue;
        if(vs!=4){ id_pI_uvec[vs]=nid++; id_pO_uvec[vs]=nid++; }
        id_pI_uarr3[vs]=nid++;
    }
    ...
    /* Function body — unrolled 3 vertices */
    uint32_t id_fnty=nid++, id_main=nid++, id_label=nid++;
    /* Per vertex: pos_ptr, pos_val, opos_ptr + 2 per user var */
    uint32_t id_bdy[3][3 + MAX_USER_VARS*2];
    memset(id_bdy, 0, sizeof(id_bdy));
    for(int v=0;v<3;v++)
        for(int k=0;k<(3 + n_uvars*2);k++) id_bdy[v][k]=nid++;

    uint32_t bound=nid;

    SB b; if(!sb_i(&b, 350 + n_uvars*50)) return false;
    ...
    /* ExecutionModes */
    {uint32_t w[]={OP(16,3),id_main,22}; sb_pn(&b,w,3);}   /* Triangles=22         */
    {uint32_t w[]={OP(16,3),id_main,29}; sb_pn(&b,w,3);}   /* OutputTriangleStrip=29 */
    {uint32_t w[]={OP(16,4),id_main,26,3}; sb_pn(&b,w,4);} /* OutputVertices 3     */

    /* Decorations */
    {uint32_t w[]={OP(71,3),id_pv_in,2}; sb_pn(&b,w,3);}          /* Block */
    {uint32_t w[]={OP(72,5),id_pv_in,0,11,0}; sb_pn(&b,w,5);}     /* member 0 = Position */
    {uint32_t w[]={OP(71,3),id_pv_out,2}; sb_pn(&b,w,3);}
    {uint32_t w[]={OP(72,5),id_pv_out,0,11,0}; sb_pn(&b,w,5);}
    for(int u=0;u<n_uvars;u++){
        {uint32_t w[]={OP(71,4),id_uvi[u],30,uvars[u].location}; sb_pn(&b,w,4);}
    ...
    /* Block struct types */
    {uint32_t w[]={OP(30,3),id_pv_in,id_v4}; sb_pn(&b,w,3);}
    {uint32_t w[]={OP(30,3),id_pv_out,id_v4}; sb_pn(&b,w,3);}
    {uint32_t w[]={OP(28,4),id_arr_in,id_pv_in,id_c3}; sb_pn(&b,w,4);}
    ...
    /* Unrolled: emit 3 vertices */
    uint32_t vert_idx[3]={id_i0,id_i1,id_i2};
    for(int v=0;v<3;v++){
        uint32_t vi=vert_idx[v];
        uint32_t pp=id_bdy[v][0], pv=id_bdy[v][1], op=id_bdy[v][2];
        /* gl_Position: load from gl_in[v].member0, store to gl_out.member0 */
        {uint32_t w[]={OP(65,6),id_pIv4,pp,id_gl_in,vi,id_i0}; sb_pn(&b,w,6);}
        {uint32_t w[]={OP(61,4),id_v4,pv,pp}; sb_pn(&b,w,4);}
        {uint32_t w[]={OP(65,5),id_pOv4,op,id_gl_out,id_i0}; sb_pn(&b,w,5);}
        {uint32_t w[]={OP(62,3),op,pv}; sb_pn(&b,w,3);}
        /* User varyings */
        for(int u=0;u<n_uvars;u++){
            uint32_t vs=uvars[u].vec_size, tv=id_uvec[vs], pI=id_pI_uvec[vs];
            uint32_t pO=(vs==4)?id_pOv4:id_pO_uvec[vs];
            uint32_t uptr=id_bdy[v][3+u*2], uval=id_bdy[v][3+u*2+1];
        ...
        {uint32_t w[]={OP(218,1)}; sb_pn(&b,w,1);}  /* EmitVertex */
    }
    {uint32_t w[]={OP(219,1)}; sb_pn(&b,w,1);}  /* EndPrimitive */
    {uint32_t w[]={OP(253,1)}; sb_pn(&b,w,1);}  /* Return */
    {uint32_t w[]={OP(56,1)}; sb_pn(&b,w,1);}   /* FunctionEnd */
